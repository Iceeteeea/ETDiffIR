...
class SDE(abc.ABC):
    ...
    @abc.abstractmethod
    def drift(self, x, t): #这个应该是f(x,t)
        pass

    @abc.abstractmethod
    def dispersion(self, x, t): # 这个应该是gt
        pass
...
class IRSDE(SDE):
    ...
    def _initialize(self, max_sigma, T, schedule, eps=0.01):
        ...
        def constant_theta_schedule(timesteps, v=1.): #每一步都是1
            """
            constant schedule
            """
    ...
    def reverse_sde(self, xt, T=-1, save_states=False, save_dir='sde_state', **kwargs):
        ...
        x = xt.clone()
        for t in tqdm(reversed(range(1, T + 1))): ###起始值原来为1
