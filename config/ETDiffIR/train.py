...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            LQ, GT, deg_type, text = train_data["LQ"], train_data["GT"], train_data["type"], train_data["text"]
            deg_token = tokenizer(deg_type).to(device)
            text_token = tokenizer(text).to(device)
            img4clip = train_data["LQ_clip"].to(device)
            with torch.no_grad(), torch.cuda.amp.autocast():
                image_context, degra_context = clip_model.encode_image(img4clip, control=True) #从DACLIP得到损伤和prompt
                text_context = clip_model.encode_text(text_token, normalize=True) #搞清楚这边该传入的text是什么数据类型
                text_context = text_context.float()
                image_context = image_context.float()
                degra_context = degra_context.float()
                # new_text_context = torch.cat([text_context, degra_context], dim=1)
                new_text_context = text_context + degra_context

            timesteps, states = sde.generate_random_states(x0=GT, mu=LQ)

            model.feed_data(states, LQ, GT, text_context=new_text_context, image_context=text_context) # xt, mu, x0
            ...
            if current_step % opt["logger"]["print_freq"] == 0:
                ...
                message = "<{},epoch:{:3d}, iter:{:8,d}, lr:{:.3e}> ".format(
                    current_time, epoch, current_step, model.get_current_learning_rate()
                )
