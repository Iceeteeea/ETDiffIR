...
from .module_util import (
    ...
    Upsample, Downsample,
    default_conv,
    ResBlock, Upsampler,newResBlock2,convNetBlock,
...
class myConditionalUNet2(nn.Module):
    def __init__(self, in_nc, out_nc, nf, ch_mult=[1, 2, 4, 8],n_filts=32,
        ...
        block_class = functools.partial(newResBlock2, conv=default_conv,act=NonLinearity(), is_right=False)
        block_class2 = functools.partial(newResBlock2, conv=default_conv,act=NonLinearity(), is_right=True)
        ...
        self.patch_embed = convNetBlock(nf)
    ...
    def forward(self, xt, cond, time, text_context=None, image_context=None):
        ...
        h = []
        x = self.patch_embed(x) # 1, 64, 256, 256
...
if __name__ == '__main__':
    ...
    model = myConditionalUNet2(in_nc=3, out_nc=3, nf=64, ch_mult=[1, 2, 4, 8], context_dim=512, use_degra_context=True, use_image_context=True, n_filts=64)
    model.to(device)
    img = torch.ones([4, 3, 400, 400])
    img = img.to(device)
    cond = torch.randn([4, 3, 400, 400])
    cond = cond.to(device)
    time = 5
