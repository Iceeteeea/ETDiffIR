...
class HANCLayer(torch.nn.Module):
    ...
    def forward(self, inp):
        ...
        x = x.contiguous().view(batch_size, num_channels * (2 * self.k - 1), H, W)
...
class HANCBlock(torch.nn.Module):
    ...
    def forward(self, inp, scale_shift=None):
...
class ACC_UNet(torch.nn.Module):
    ...
    def __init__(self, n_channels, n_classes, nf, n_filts=32, context_dim=512, use_degra_context=True, use_image_context=False):
        """
        Initialization
